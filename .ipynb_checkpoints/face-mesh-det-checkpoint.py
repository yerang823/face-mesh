...
def lip_seg2(vid_path1, vid_path2):
    res = []
    res2 = []
    #lips_alpha = []
    inter_lips_alpha = []
    ...
    with mp_face_mesh.FaceMesh( max_num_faces=1,
        ...
        while cap.isOpened():
            ...
            if h!=h2 or w!=w2:
                print('image size not equal!!', h,w, h2,w2)
                break
...
#             image2 = copy.deepcopy(image)
            ...
            mask2 = np.zeros(image2.shape)
            
            # <<<< 이미지에서 얼굴들 detection >>>>
            
            det_res = bbox_arr[idx]
            #with mp_face_detection.FaceDetection(model_selection=1, min_detection_confidence=0.5) as face_detection:
            #    det_res = face_detection.process(image) # detection 결과를 vid_path1, vid_path2 에서 공유하기 위함!!
            ...
            for i in range(len(det_res)):
                ...
                if mesh_res.multi_face_landmarks:                        
                    ...
                    for face_landmarks in mesh_res.multi_face_landmarks:
                        ...
                        cropped_img = np.zeros(im.shape, dtype=np.uint8)
                        cropped_img[Y, X] = 255 #image[Y, X]
                    mask[y1:y2, x1:x2] = cropped_img

                # ======================
                # image 2
                # ======================
                im = image2[y1:y2, x1:x2]       
                mesh_res = face_mesh.process(im)
                ...
                # post process
                # ======================
                res_img = postproc(image, image2, mask, mask2)
                #res.append(res_img)
...
#             res.append(image)
#             res2.append(image2)
#             inter_lips_alpha.append(mask)
#             inter_lips_alpha2.append(mask2)
            idx+=1
...
#     return (inter_lips_alpha, res)
#     return (res,res2, inter_lips_alpha, inter_lips_alpha2)
    return res
...
def postproc(image, image2, mask, mask2):
    ...
    foreground_mouth = image2.astype(float)

    mask3 = (mask.astype(np.int32) + mask2.astype(np.int32)).clip(0,255).astype(np.uint8)
    ...
    foreground_mouth = cv2.multiply(alpha_mouth, foreground_mouth)
    outImage2 = cv2.multiply(1.0 - alpha_mouth, background)
    result_image = cv2.add(foreground_mouth, outImage2)
...
def main(sim_vid_out, sber_vid_out, src_wv_dir, out_dir):
    ...
    for i in range(len(sim_li)):
        ...
        wv_src_path = glob.glob(f'{src_wv_dir}/{src_name}*.mp4')[0]
        save_wv_path = glob.glob(f'./src_wave/{src_name}*.wav')[0]
...
#         inter_lips_alpha, res = lip_seg(vid_path1)
#         inter_lips_alpha2, res2 = lip_seg(vid_path2)
        #res,res2,inter_lips_alpha,inter_lips_alpha2 = lip_seg2(vid_path1, vid_path2)
        res_arr = lip_seg2(vid_path1, vid_path2)
...
#         blending_res = blend(res, res2, lips_alpha, inter_lips_alpha)
        ...
        write_video(res_arr, save_wv_path, 30, save_vid_path, slow_write=True) # true 로 바꿔보기
